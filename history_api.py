...
class HistoryAPI:
    ...
    def __init__(self, data_dir=None):
        ...
        self.altseason_history_file = os.path.join(self.data_dir, "altseason_history.json")
    ...
    def _ensure_history_files_exist(self):
        ...
        for file_path in [self.rank_history_file, self.fear_greed_history_file, self.trends_history_file, self.altseason_history_file]:
    ...
    def save_altseason_index_history(self, signal, description, status, index, btc_performance):
        """
        Сохраняет новые данные Altcoin Season Index в историю
        
        Args:
            signal (str): Emoji-сигнал (🔴 🟠 🟡 🟢)
            description (str): Текстовое описание сигнала
            status (str): Текстовый статус (No Altseason, Weak Altseason, Moderate Altseason, Strong Altseason)
            index (float): Значение индекса (0.0 до 1.0)
            btc_performance (float): Производительность Bitcoin за выбранный период (в %)
            
        Returns:
            dict: Запись истории данных Altcoin Season Index
        """
        try:
            ...
            # Загружаем существующую историю
            history = self._load_history(self.altseason_history_file)
            
            # Добавляем новую запись
            history.append(entry)
            
            # Сохраняем обновленную историю
            if self._save_history(self.altseason_history_file, history):
                logger.info(f"Saved new Altcoin Season Index history entry: {signal} - {status} ({index})")
                return entry
            else:
                return None
            
        except Exception as e:
            logger.error(f"Failed to save Altcoin Season Index history: {str(e)}")
            return None
    ...
    def get_altseason_index_history(self, limit=100, offset=0):
        """
        Получает историю данных Altcoin Season Index, отсортированную по времени (новые сначала)
        ...
        Returns:
            list: Список записей истории данных Altcoin Season Index
        """
        try:
            history = self._load_history(self.altseason_history_file)
            ...
            logger.error(f"Failed to get Altcoin Season Index history: {str(e)}")
            return []