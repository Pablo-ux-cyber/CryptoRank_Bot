...
class AltcoinSeasonIndex:
    ...
    def format_altseason_message(self, altseason_data=None):
        ...
        # Максимально лаконичный формат сообщения с иконкой
        # Название индикатора + иконка + четкое указание текущего режима рынка
        index_percent = int(altseason_data['index']*100)
        
        # Определяем режим рынка и добавляем соответствующую иконку
        if index_percent < 25:
            market_mode = "Bitcoin Season"
            explanation = "Altcoins underperform Bitcoin"
            icon = "₿"  # Иконка биткоина
        else:
            market_mode = "Altcoin Season"
            explanation = "Altcoins outperform Bitcoin"
            icon = "🪙"  # Иконка монеты
        
        # Формируем максимально лаконичное сообщение с иконкой
        message = f"Altcoin Season Index: {icon} {market_mode}\n{explanation}"
