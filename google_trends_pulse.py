...
class GoogleTrendsPulse:
    ...
    def format_trends_message(self, trends_data=None):
        ...
        # Получаем данные для стрелок тренда, если они есть в истории
        prev_signal = None
        change_arrow = ""
        
        # Попробуем найти предыдущую запись (не из текущего дня)
        if self.history_data and len(self.history_data) > 1:
            today = datetime.now().strftime("%Y-%m-%d")
            for entry in self.history_data:
                entry_date = entry.get("timestamp", "").split(" ")[0]  # Берем только дату
                if entry_date != today and entry.get("signal") != signal:
                    prev_signal = entry.get("signal")
                    break
            
            # Добавляем стрелку направления изменения, если есть предыдущий сигнал
            if prev_signal:
                # Определяем направление изменения (улучшение/ухудшение)
                # Здесь мы будем использовать упрощенную логику: 
                # 🟢 лучше чем 🟡 лучше чем ⚪️ лучше чем 🟠 лучше чем 🔴
                signal_weights = {
                    "🟢": 5,  # Наилучший сигнал
                    "🟡": 4,
                    "⚪": 3,
                    "🟠": 2,
                    "🔴": 1,  # Наихудший сигнал
                    "🔵": 0   # Особый случай - низкий интерес
                }
                
                current_weight = signal_weights.get(signal, 3)
                prev_weight = signal_weights.get(prev_signal, 3)
                
                if current_weight > prev_weight:
                    change_arrow = " 🔼"  # Улучшение
                elif current_weight < prev_weight:
                    change_arrow = " 🔽"  # Ухудшение
                # Если веса равны, стрелку не добавляем
        
        # Форматируем сообщение только для реальных данных API с добавлением стрелки тренда
        message = f"{signal} Google Trends:{change_arrow} {description}"
        return message
