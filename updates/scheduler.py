    ...
    def _scheduler_loop(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                # Проверяем, не устарел ли файл блокировки (старше 30 минут)
                if os.path.exists(manual_lock_file):
                    try:
                        file_time = os.path.getmtime(manual_lock_file)
                        current_time = time.time()
                        # Если файл старше 30 минут, удаляем его
                        if current_time - file_time > 30 * 60:
                            os.remove(manual_lock_file)
                            logger.info(f"Удален устаревший файл блокировки ручной операции (возраст: {(current_time - file_time)/60:.1f} минут)")
                    except Exception as e:
                        logger.warning(f"Ошибка при проверке старого файла блокировки ручной операции: {str(e)}")
